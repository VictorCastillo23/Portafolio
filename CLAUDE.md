...
- Components are split into `components/layout` (Nav, Footer, sidebars), `components/sections` (page sections: Hero, About, Credentials, Experience, Projects, Contact), `components/ui` (shared primitives: Icon, Section, ProjectCard), and `components/chat` (the chat widget: `ChatWidget`, `ChatMessage`, `useChatStream` — a self-contained, multi-file feature with its own hook, kept out of the flat single-file `components/layout` chrome).
- The chat assistant has no RAG: `lib/chat/knowledge.ts` builds its whole knowledge base from `data/content.ts` + the merged projects, and `app/api/chat/route.ts` sends it in the system prompt on every request. `lib/chat/knowledge.test.ts` guards the size (`KNOWLEDGE_CHAR_BUDGET`).
